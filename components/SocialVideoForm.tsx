...
import { Loader2, Link2, Download, Clock, Music, Film } from "lucide-react";
...
export default function SocialVideoForm({
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(`${API_URL}/social/info`, {
        method: "POST",
  ...
  function buildDownloadHref(formatId: string, ext: "mp4" | "mp3") {
    ...
    return `${API_URL}/social/download?url=${encodeURIComponent(url)}&format_id=${encodeURIComponent(
      formatId
        ...
        <div className="mx-auto mt-14 max-w-3xl">
          <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg">
            <div className="grid md:grid-cols-[280px_1fr]">
              <div className="flex aspect-video w-full self-start items-center justify-center overflow-hidden bg-slate-100">
                {info.thumbnail ? (
                  <img
                    src={info.thumbnail}
                    alt={info.title}
              ...
              <div className="flex flex-col justify-between p-8">
                <div>
                  ...
                  {info.uploader && (
                    <p className="mt-3 text-slate-500">{info.uploader}</p>
                  )}
